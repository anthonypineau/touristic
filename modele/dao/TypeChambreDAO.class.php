...
class TypeChambreDAO {
  ...
  /**
   ...
   /**
    ...
    /**
     * Retourne la liste de tous les types de chambres et du nombre d'attributions de chacun
     * @return array tableau à 2 dimensions : 
     *      - dimension 2, index "typeChambre" => objet de type TypeChambre
     *      - dimension 2, index "nbAttrib" => nombre d'attributions pour ce type de chambre
     */
    public static function getAllWithNbAttributions() {
        $lesTypesChambresAvecNbAttrib = array();
        $requete = "SELECT * FROM TypeChambre";
        $stmt = Bdd::getPdo()->prepare($requete);
        $ok = $stmt->execute();
        if ($ok) {
            ...
            while ($enreg = $stmt->fetch(PDO::FETCH_ASSOC)) {
                // instancier un TypeChambre et l'ajouter au tableau
                /* @var TypeChambre $unTC */
                $unTC = self::enregVersMetier($enreg);
                $lesTypesChambresAvecNbAttrib[]["typeChambre"] = $unTC;
                $lesTypesChambresAvecNbAttrib[$i]["nbAttrib"] = count(AttributionDAO::getAllByIdTypeChambre($unTC->getId()));
                $i++;
            }
        }
        return $lesTypesChambresAvecNbAttrib;
    }
